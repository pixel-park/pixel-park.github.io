...
function Project(props: imageProps) {
  ...
  useEffect(() => {
    const overalHeight =
      hiddenP.current &&
      Array.from(hiddenP.current.children).reduce(
        (ocum, curr) => ocum + curr.getBoundingClientRect().height,
        0
      );
    setHeight(Math.ceil(overalHeight ? overalHeight : 0));
    if (more && hiddenP.current)
      hiddenP.current.style.height = paragHeight + "px";
  ...
  return (
    <div>
      <div className="project-title">
        {props.title.url ? (
          <div className="project-title_wrapper">
            <a href={props.title.url} target="_blank" rel="noreferrer">
              {props.title.title}
            </a>
            <p>({props.title.created})</p>
          </div>
        ) : (
        ...
        style={
          {
            ...
            "--iHeightMin": props.iFrame?.height
              ? props.iFrame?.height[0] + "px"
              : "0px",
            "--iHeightMax": props.iFrame?.height
              ? props.iFrame?.height[1] + "px"
              : "0px",
      ...
      >
        ...
        <p>{props.textA}</p>
        {props.iFrame && props.screenSize > props.iFrame.minWidth && (
          <iframe src={props.iFrame.url} title={props.title.title}></iframe>
        )}
          ...
          <>
            <div className="hidden-text__container" ref={hiddenP}>
              <div>
                ...
                {props.expand.link && (
                  <a
                    href={props.expand.link.url}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {props.expand.link.title}
                  </a>
                )}
              </div>
            </div>
